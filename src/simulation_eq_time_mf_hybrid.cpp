...
int main(int argc, char **argv)
{
    ...
    double frac = atof(argv[3]);  //fraction treated in mean field
    ...
    std::ifstream nUpData("nUp.csv");
    std::ifstream nDwData("nDw.csv");
    Eigen::VectorXd nUp = Eigen::VectorXd(NX * NY * NORB);
    Eigen::VectorXd nDw = Eigen::VectorXd(NX * NY * NORB);
    ...
    int idx = 0;
    while (nUpData && idx < NX * NY * NORB)
    {
        // read stuff from the file into a string and print it
        std::string strInput;
        getline(nUpData, strInput);
        nUp(idx) = stod(strInput);
        idx++;
    }
    idx = 0;
    while (nDwData && idx < NX * NY * NORB)
    {
        // read stuff from the file into a string and print it
        std::string strInput;
        getline(nDwData, strInput);
        nDw(idx) = stod(strInput);
        idx++;
    }
    ...
    double nu = pow( ( (1 - frac) * U * dt), 0.5)
    + pow( ( (1 -frac ) * U * dt), 1.5) / 12;
    ...
    #pragma omp parallel num_threads(NTH)
    {
        ...
        Geometry< NSITES > K;
        K.setParams(tmd);
        K.HoppingMatrixMF(U_MF, nUp, nDw);
    ...
    write(L, totalMCSweeps, W, A, av_sign, av_weights,
      elDens, (1-frac) * U, elDoubleOc, kineticEnergy, spin_corr);
